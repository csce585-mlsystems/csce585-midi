...
if train_model "lstm_generator" "training/train_generator.py" \
    "--model_type lstm --epochs 20 --batch_size 64 --lr 0.001 --hidden_size 256 --num_layers 2 --dataset miditok"; then
    ((SUCCESS_COUNT++))
else
    ((FAIL_COUNT++))
fi
...
if train_model "gru_generator" "training/train_generator.py" \
    "--model_type gru --epochs 20 --batch_size 64 --lr 0.001 --hidden_size 256 --num_layers 2 --dataset miditok"; then
    ((SUCCESS_COUNT++))
else
    ((FAIL_COUNT++))
fi
...
if train_model "transformer_generator" "training/train_generator.py" \
    "--model_type transformer --epochs 15 --batch_size 32 --lr 0.0001 --d_model 256 --nhead 8 --num_layers 4 --dataset miditok"; then
    ((SUCCESS_COUNT++))
else
    ((FAIL_COUNT++))
fi
...
if train_model "lstm_large_generator" "training/train_generator.py" \
    "--model_type lstm --epochs 15 --batch_size 32 --lr 0.001 --hidden_size 512 --num_layers 3 --dataset miditok"; then
    ((SUCCESS_COUNT++))
else
    ((FAIL_COUNT++))
fi
